...
export default function SignupPage() {
  ...
  // ---------------------------
  useEffect(() => {
    loadCaptchaEnginge(6); // 6글자 랜덤 보안문자
    ...
    if (!(window.daum && window.daum.Postcode)) {
      ...
      // cleanup: 페이지 나갈 때 스크립트 제거해줄 수도 있음
      return () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!valid.birth)
      return alert("생년월일(YYYYMMDD)을 확인해주세요.");
    if (!valid.email) return alert("이메일을 확인해주세요.");
    if (!valid.emailAuth)
      return alert("이메일 인증을 완료해주세요.");

    // 선택 항목은 비어 있어도 통과 (요구사항 기준)
    // 단 값이 들어왔는데 경고가 남아 있으면 막는 게 UX적으로 더 낫다.
    ...
    try {
      ...
      alert("회원가입이 완료되었습니다!");
      navigate("/login"); // 가입 후 로그인 페이지로 이동한다고 가정
    } catch {
      // request()에서 공용 alert는 이미 한 번 띄워주고 있음
    }
  };
